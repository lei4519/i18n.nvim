...
function M.get_completions(self, ctx, callback)
    -- 调试：如果需要查看 ctx 的结构，可以取消注释下面的行
    -- vim.print("blink.cmp ctx:", ctx)
    ...
    -- 获取当前行和列
    -- blink.cmp 的上下文可能有不同的字段名
    local line = ctx.line or ctx.cursor_line
    local col = (ctx.cursor and ctx.cursor[2]) or ctx.col or ctx.cursor_col

    -- 如果还是 nil，尝试从 bufnr 获取
    if not line or not col then
        local cursor = vim.api.nvim_win_get_cursor(0)
        local row = cursor[1]
        col = cursor[2] + 1 -- nvim_win_get_cursor 返回 0-based，我们需要 1-based
    ...
    -- 获取补全项和 key-value 映射
    local items, key_values = get_completion_items()
        ...
        if vim.startswith(key, prefix) then
            ...
            if next_level and not seen[next_level] then
                ...
                table.insert(filtered, {
                    ...
                    kind = is_leaf and vim.lsp.protocol.CompletionItemKind.Constant or
